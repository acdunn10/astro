...
import ephem
from astro import astro_config
    ...
    if url is not None:
        try:
            ...
            if r.status_code == 200:
                ...
                config.read(astro_config('config.ini'))
                config.set('ctwin', 'latitude', str(data['latitude']))
                config.set('ctwin', 'longitude', str(data['longitude']))
                with open(astro_config('config.ini'), 'w') as f:
                    config.write(f)
...
def get_location():
    config = configparser.ConfigParser()
    config.read(astro_config('config.ini'))
    if not config.has_section('ctwin'):
        config.add_section('ctwin')
        observer = ephem.city('Columbus')
        config.set('ctwin', 'latitude', str(observer.lat))
        config.set('ctwin', 'longitude', str(observer.lon))
        with open(astro_config('config.ini'), 'w') as f:
            config.write(f)
    return {
        'lat': config.get('ctwin', 'latitude'),
        'lon': config.get('ctwin', 'longitude'),
        }


if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO,
        filename=astro_config('ctwin.log'),
